...
describe("FinancialFormParser", () => {
  ...
  beforeAll(() => {
    const fixturePath = join(
      __dirname,
      "../../tests/fixtures/forms/10-K/apple_10k_2023.txt",
    );
  ...
  describe("parseAll", () => {
    ...
    it("should calculate financial metrics", () => {
      ...
      expect(
        typeof metrics.returnOnEquity === "number" ||
          metrics.returnOnEquity === null,
      ).toBe(true);
      expect(
        typeof metrics.currentRatio === "number" ||
  ...
  describe("getFinancialStatements", () => {
    it("should return structured financial data", () => {
      ...
      expect(statements.balanceSheet.liabilities).toHaveProperty(
        "currentLiabilities",
      );

      expect(Array.isArray(statements.balanceSheet.assets.currentAssets)).toBe(
        true,
      );
      expect(Array.isArray(statements.incomeStatement.operatingExpenses)).toBe(
        true,
      );
    });
  });
