...
describe("XBRLInstance", () => {
  ...
  describe("getConceptValue", () => {
    it("should extract concept value", async () => {
      const assets = await xbrlInstance.getConceptValue(
        "Assets",
        "us-gaap",
        "USD",
      );
      ...
      const revenue = await xbrlInstance.getConceptValue(
        "Revenues",
        "us-gaap",
        "USD",
      );
    ...
    it("should return null for non-existent concept", async () => {
      const value = await xbrlInstance.getConceptValue(
        "NonExistentConcept",
        "us-gaap",
        "USD",
      );
    ...
    it("should handle specific period", async () => {
      ...
      const noValue = await xbrlInstance.getConceptValue(
        "Assets",
        "us-gaap",
        "USD",
        "2022-09-30",
  ...
  describe("getBalanceSheet", () => {
    it("should extract balance sheet data", async () => {
      ...
      if (balanceSheet.liabilities.total_liabilities) {
        expect(balanceSheet.liabilities.total_liabilities.value).toBe(
          290437000000,
        );
      }
