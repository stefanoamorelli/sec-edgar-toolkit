...
export class FinancialFormParser {
  ...
  parseBalanceSheet(): BalanceSheet {
    const balanceSheetSection = this.extractSection(
      "BALANCE SHEET|CONSOLIDATED BALANCE SHEET",
    );

    return {
      assets: {
        currentAssets: this.extractBalanceSheetItems(
          balanceSheetSection,
          "current.*assets",
        ),
        nonCurrentAssets: this.extractBalanceSheetItems(
          balanceSheetSection,
          "non.?current.*assets|property.*plant.*equipment",
        ),
        totalAssets: this.extractBalanceSheetItem(
          balanceSheetSection,
          "total.*assets",
        ),
      },
      liabilities: {
        currentLiabilities: this.extractBalanceSheetItems(
          balanceSheetSection,
          "current.*liabilities",
        ),
        nonCurrentLiabilities: this.extractBalanceSheetItems(
          balanceSheetSection,
          "non.?current.*liabilities|long.?term.*debt",
        ),
        totalLiabilities: this.extractBalanceSheetItem(
          balanceSheetSection,
          "total.*liabilities",
        ),
      },
      equity: {
        totalEquity: this.extractBalanceSheetItem(
          balanceSheetSection,
          "total.*equity|shareholders.*equity",
        ),
        retainedEarnings: this.extractBalanceSheetItem(
          balanceSheetSection,
          "retained.*earnings",
        ),
      },
  ...
  parseIncomeStatement(): IncomeStatement {
    const incomeSection = this.extractSection(
      "INCOME STATEMENT|CONSOLIDATED STATEMENT.*OPERATIONS|STATEMENT.*EARNINGS",
    );

    return {
      revenue: this.extractIncomeStatementItem(
        incomeSection,
        "revenue|net.*sales|total.*revenue",
      ),
      grossProfit: this.extractIncomeStatementItem(
        incomeSection,
        "gross.*profit|gross.*margin",
      ),
      operatingIncome: this.extractIncomeStatementItem(
        incomeSection,
        "operating.*income|income.*from.*operations",
      ),
      netIncome: this.extractIncomeStatementItem(
        incomeSection,
        "net.*income|net.*earnings",
      ),
      earningsPerShare: this.extractIncomeStatementItem(
        incomeSection,
        "earnings.*per.*share|basic.*earnings.*per.*share",
      ),
      operatingExpenses: this.extractIncomeStatementItems(
        incomeSection,
  ...
  parseCashFlowStatement(): CashFlowStatement {
    const cashFlowSection = this.extractSection(
      "CASH FLOW|CONSOLIDATED STATEMENT.*CASH.*FLOW",
    );

    return {
      operatingActivities: this.extractCashFlowItems(
        cashFlowSection,
        "operating.*activities|cash.*from.*operations",
      ),
      investingActivities: this.extractCashFlowItems(
        cashFlowSection,
        "investing.*activities|cash.*from.*investing",
      ),
      financingActivities: this.extractCashFlowItems(
        cashFlowSection,
        "financing.*activities|cash.*from.*financing",
      ),
      netCashFlow: this.extractCashFlowItem(
        cashFlowSection,
        "net.*cash.*flow|net.*increase.*decrease.*cash",
      ),
    };
  }
  ...
  getBusinessSegments(): BusinessSegment[] {
    const segmentSection = this.extractSection(
      "SEGMENT|BUSINESS.*SEGMENT|GEOGRAPHIC.*INFORMATION",
    );
    const segments: BusinessSegment[] = [];

    // Extract segment data using pattern matching
    const segmentPattern =
      /(\w+(?:\s+\w+)*)\s+segment.*?revenue.*?\$?([\d,]+)/gi;
    let match;
  ...
  getManagementDiscussion(): MDSection[] {
    const mdaSection = this.extractSection(
      "MANAGEMENT.*DISCUSSION|ITEM 2|MD&A",
    );
    const sections: MDSection[] = [];

    // Split into subsections
    const subsections = mdaSection.split(
      /(?=OVERVIEW|RESULTS OF OPERATIONS|FINANCIAL CONDITION|LIQUIDITY)/i,
    );
  ...
  getXBRLFacts(): XBRLFact[] {
    ...
    const xbrlPattern =
      /<ix:nonFraction[^>]*name="([^"]*)"[^>]*contextRef="([^"]*)"[^>]*decimals="([^"]*)"[^>]*unitRef="([^"]*)"[^>]*>([^<]*)</gi;
    let match;
  ...
  private extractSection(pattern: string): string {
    ...
    return this.rawContent.substring(
      startIndex,
      endIndex > 0 ? endIndex : startIndex + 50000,
    );
  }
  ...
  private extractBalanceSheetItems(
    ...
    const regex = new RegExp(pattern + ".{0,300}?\\$([\\d,]+)", "gi");
    let match;
  ...
  private parseHeader() {
    ...
    const filingDateMatch = this.rawContent.match(
      /FILED AS OF DATE:\s*(\d{8})/,
    );
    const periodMatch = this.rawContent.match(
      /CONFORMED PERIOD OF REPORT:\s*(\d{8})/,
    );
  ...
  private extractKeyMetrics(
    ...
    const changePattern =
      /(\w+(?:\s+\w+)*)\s+(?:increased|decreased|changed)\s+by\s+([\d.]+%)/gi;
    let match;
  ...
  private calculateCurrentRatio(balanceSheet: BalanceSheet): number | null {
    ...
    const currentLiabilities =
      balanceSheet.liabilities.currentLiabilities.reduce(
