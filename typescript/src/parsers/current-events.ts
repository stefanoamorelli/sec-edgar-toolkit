...
export class CurrentEventParser {
  ...
  getMaterialAgreements(): Agreement[] {
    ...
    const agreementSection = this.extractSection(
      "ITEM 1.01|MATERIAL AGREEMENT|DEFINITIVE AGREEMENT",
    );
    ...
    if (agreementSection) {
      ...
      const contentAfterHeader = agreementSection.replace(
        /^ITEM\s+\d+\.\d+[^\n\r]*[\n\r]+/i,
        "",
      );
      const typeMatch = contentAfterHeader.match(
        /(?:entered into|executed|signed)\s+(?:a|an|the)?\s*([A-Za-z\s]+Agreement)/i,
      );
      const agreementType = typeMatch
        ? typeMatch[1].trim()
        : "Material Agreement";

      // Find counterparty
      const withPattern =
        /(?:with|between.*?and)\s+([A-Za-z\s]+(?:Inc\.|Corp\.|Technologies|Company|LLC)?)[\s,.(]/i;
  ...
  getExecutiveChanges(): ExecutiveChange[] {
    ...
    const changeSection = this.extractSection(
      "ITEM 5.02|DEPARTURE.*DIRECTOR|APPOINTMENT.*OFFICER",
    );

    if (changeSection) {
      // Look for departures/resignations
      const departurePattern =
        /(?:\(b\))\s*On\s+[A-Za-z]+ \d+, \d+,\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+),.*?(?:notified|informed|announced|resignation|retire|departure)/gi;
      let match;
      ...
      const appointmentPattern =
        /(?:appointed|named)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)(?:,|\s+as|\s+to)/gi;
  ...
  getAcquisitions(): Acquisition[] {
    ...
    const acquisitionSection = this.extractSection(
      "ITEM 2.01|ACQUISITION|MERGER|DIVESTITURE",
    );
  ...
  getEarningsResults(): EarningsData | null {
    ...
    const earningsSection = this.extractSection(
      "ITEM 2.02|RESULTS.*OPERATIONS|EARNINGS",
    );
    ...
    return {
      ...
      earningsPerShare: this.extractMetric(
        earningsSection,
  ...
  private extractSection(pattern: string): string {
    ...
    const nextItemMatch = this.rawContent
      .substring(startIndex + 1)
      .match(/Item\s+\d+\.\d+/i);
    const endIndex = nextItemMatch
      ? startIndex + 1 + (nextItemMatch.index || 0)
      : -1;

    return this.rawContent.substring(
      startIndex,
      endIndex > 0 ? endIndex : startIndex + 10000,
    );
  }
  ...
  private assessEventSignificance(
    description: string,
  ): "low" | "medium" | "high" {
    const highKeywords = [
      "acquisition",
  ...
  private extractPosition(text: string, name: string): string {
    ...
    const isAppointment =
      cleanText.toLowerCase().indexOf("appointed") < cleanText.indexOf(name) &&
    ...
    const patterns = isAppointment
      ? [
          ...
          new RegExp(
            `${name}(?:,\\s*age\\s*\\d+,)?.*?as\\s+(?:the\\s+)?(?:new\\s+)?([^,\\.]+?)(?:,|\\seffective|\\.)`,
            "i",
          ),
        ]
      : [
          ...
          new RegExp(
            `${name},\\s*([^,]+?)\\s*(?:,|notified|informed|announced)`,
            "i",
          ),
          // "Senior Vice President John Smith"
          new RegExp(
            `([^,]*(?:Vice President|President|Director|Officer|Manager)[^,]*),?\\s*${name}`,
            "i",
          ),
        ];
  ...
  private extractTargetName(text: string): string {
    const targetPattern =
      /(?:acquire|purchase|merger with)\s+([A-Z][^,.]{10,50})/i;
  ...
  private extractReportingPeriod(): string {
    ...
    const periodMatch = this.rawContent.match(
      /(?:quarter|period|year)\s+ended\s+([A-Za-z]+ \d+, \d+)/i,
    );
    if (periodMatch) {
      return periodMatch[1];
    }

    const conformedPeriodMatch = this.rawContent.match(
  ...
  private extractGuidance(
    ...
    const guidancePattern =
      /(?:guidance|expects?).*?([a-zA-Z\s]+).*?(\$[0-9,]+|\d+%)/gi;
    let match;
